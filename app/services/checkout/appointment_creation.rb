module Checkout
  class AppointmentCreation
    ...
    def perform
      lines_types = params[:lines_attributes].values.map { |line| line[:sellable_type] }.uniq
      if lines_types.include?('ServicePrice') && params[:appointment_id].present?
        services_ids = params[:lines_attributes].values.select { |line| line[:sellable_id] }.where(line[:sellable_type] == 'ServicePrice')
        ...
        if new_services_ids.present? || removed_services_ids.present?
          new_services = params[:lines_attributes].values.select { |line| line[:sellable_type] == 'ServicePrice' && line[:sellable_id] == new_services_ids }
    ...
    def appointment_builder
      ...
      services = params[:lines_attributes].values.select { |line| line[:sellable_type] == 'ServicePrice' }
