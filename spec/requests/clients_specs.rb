...
RSpec.describe "Clients API", type: :request do
  ...
  describe "#index" do
    ...
    context "no params" do
      ...
      xit "list all clients data" do
        get "/clients", {headers: headers}
        expect(response.code).to eq("200")
        expect(parsed_response["data"].size).to eq(3)
      end
      xit "list all clients data that match search with first name " do
        get "/clients?search=#{clients.first.first_name}", {headers: headers}
        expect(response.code).to eq("200")
        expect(parsed_response["data"].size).to eq(1)
      end
      xit "list all clients data that match search with last name " do
